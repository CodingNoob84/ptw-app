...
export const PermitDetailsPage = ({ userId, permitId }: { userId: string; permitId: string }) => {
  ...
  return (
    <Card className="max-w-4xl mx-auto">
      <CardHeader className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <CardTitle className="text-2xl">{data?.permit.project_name}</CardTitle>
          <p className="text-sm text-muted-foreground">Permit ID: {data?.permit.id}</p>
        </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                ...
                <p className="font-medium">
                  Tower {data?.permit.tower_name}, Floor {data?.permit.floor_name}, Unit{' '}
                  {data?.permit?.unit_name}
                </p>
              </div>
              <div>
                <h3 className="text-sm font-medium text-muted-foreground">Category</h3>
                <p className="font-medium">
                  {data?.permit?.category_name} - {data?.permit?.subcategory_name}
                </p>
              </div>
              ...
              <div>
                ...
                <p className="font-medium">{format(data?.permit?.updated_at ?? '', 'PPpp')}</p>
              </div>
            </div>

            <div>
              <h3 className="text-sm font-medium text-muted-foreground">Applicant Comments</h3>
