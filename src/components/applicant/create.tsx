  ...
  const handleSubmit = useCallback(
    async (e: React.FormEvent) => {
      ...
      const inputData = {
        ...
        assignedTo: userdata?.reporting_to[0]?.id || "",
        images: images,
      ...
      try {
        const response = await submitPermitData(inputData, supabase);
        if (response.success) {
          queryClient.invalidateQueries({
            queryKey: ["allpermits", userdata?.user.id],
          });
    ...
    [formData, images, queryClient, router, supabase, userdata]
  );
    ...
    <div className="container mx-auto px-4 py-10">
      ...
      <Card className="max-w-4xl mx-auto">
        <CardHeader className="px-6 py-5">
          <CardTitle className="text-2xl">Create New Permit to Work</CardTitle>
        </CardHeader>
            ...
            <form onSubmit={handleSubmit}>
              <TabsContent value="details" className="space-y-6">
                <div className="space-y-5">
                  ...
                  {isLoadingProjectDetails && selectedProjectId ? (
